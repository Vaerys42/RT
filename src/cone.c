...
double		ft_check_cone(t_cone *cone, t_ray *ray)
{
    ...
    delta = b * b - (4 * a * c);
	if (cone->obj->pln == NULL)
        return (disc_eq(a, b, c, delta));
	return (ft_inter_plane_ini(ray, cone->obj, a, b, c));
}
...
void		new_cone_dst(t_rt *rt, int type, double tmp)
{
	...
	if (type == 0)
	{
		...
		if (rt->cone->obj->cut == 1 && rt->cone->obj->pln != NULL)
		{
			rt->inter->mat->r = rt->cone->obj->pln->color->r * rt->light->amb;
			rt->inter->mat->g = rt->cone->obj->pln->color->g * rt->light->amb;
			rt->inter->mat->b = rt->cone->obj->pln->color->b * rt->light->amb;
		}
	}
	if (type == 1 && rt->inter->obj == CON)
	{
		...
		if (rt->cone->obj->cut == 1)
			rt->inter->angle->dir = ft_mult_vect(-1, rt->cone->obj->pln->norm);	
