...
double		ft_check_parabol(t_parabol *parabol, t_ray *ray)
{
	...
	b = 2 * (scal(ray->dir,ray->obj) - scal(ray->dir, parabol->dir) * (scal(ray->obj, parabol->dir) + 2 * parabol->rad1));
	c = scal(ray->obj, ray->obj) - scal(ray->obj, parabol->dir) * (scal(ray->obj, parabol->dir) + 4 * parabol->rad1);
...
void		light_parabol_inter(t_rt *rt)
{
	...
	//rt->inter->angle->dir = ft_sub_vect(rt->inter->point, rt->parabol->o);
	//if (rt->inter->point.y < rt->parabol->o.y)
